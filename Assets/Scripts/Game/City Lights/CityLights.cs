...
public class CityLights : MonoBehaviour
{
    ...
    public void Start()
    {
        Init(heightProcessor.processedHeightMap, sunLight);
    }
    public void Init(RenderTexture heightMap, Light sunLight)
	{
		mesh = IcoSphere.Generate(meshRes, 0.5f).ToMesh();
		this.sunLight = this.sunLight?this.sunLight: sunLight;
	...
	void Update()
	{
		if (drawLights)
		{
			Vector3 dirToLight = -sunLight.transform.forward;
	...
	{
		r.material.SetVector(ShaderPropertyNames.dirToSunID, -sunLight.transform.forward);
